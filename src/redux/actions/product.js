...
export const actFetchProductsRequest = (token, offset) => {
  ...
  return dispatch => {
    ...
    return new Promise((resolve, reject) => {
      callApi(`products?limit=${limit}&page=${newOffset}&sortBy=-createdAt`, 'GET', null)
        .then(res => {
...
export const actFindProductsRequest = (token, searchText) => {
  ...
  return new Promise((resolve, reject) => {
    if (searchText !== undefined && searchText !== null && searchText !== '') {
      callApi(`products//search-product?search=${searchText}`, 'GET', null, token)
      .then(res => {
        if (res && res.status === 200) {
...
export const actGetProductRequest = (token, id) => {
  return async dispatch => {
    await callApi(`products/get-product-by-id?ProductId=${id}`, 'GET', null, token);
  };
}
